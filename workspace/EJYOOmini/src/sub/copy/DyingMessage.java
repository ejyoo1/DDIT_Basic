...
public class DyingMessage{
	static Novelist nl = new Novelist();
	...
	public static void main (String[] args) {//메인메서드 카드 목록과 메뉴 세팅
		...
		while(true) {
			...
			if(nl.getHintcount()==5) {
				dm.userMenuSettings();//메뉴 목록을 출력하는 메서드
				break;
			}
			System.out.println ("■■■■■■■■■■■■■■■■■■■■■■■■■■■■"+nl.getHintcount()+"번째 판입니다.■■■■■■■■■■■■■■■■■■■■■■■■■■■■");
	...
	static String userMenu() {//제공된 힌트수에 따라 힌트 제한,//힌트별 메뉴 세팅
		String userSetMenu;
		if(nl.getHintcount()==1 || nl.getHintcount()==2 || nl.getHintcount()==3|| nl.getHintcount()==4 ) {//힌트를 3회까지 얻을 수 있음.
			userSetMenu = "☞☞☞당신은 힌트를 얻겠습니까? 정답을 맞추시겠습니까? (힌트얻기 / 정답맞추기 / 정답보기 / 게임종료)";
		}else {//힌트 3회를 모두 소진하는 경우]
			System.out.println (nl.getHintcount());
	...
	void userMenuSettings() {//메뉴 목록을 세팅해주는 메서드,//사용자 선택 메뉴에 따른 작업 호출
		...
		switch(userInput1) {
			case "힌트얻기" :
				nl.setHintcount (hints.userHint(nl.getHintcount()));
				break;
			case "정답맞추기" :
				users.userSuccess ();
				break;
