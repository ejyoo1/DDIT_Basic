...
public class User {
	boolean criminalVerification(String[] userAnswer) {//범인 카드 목록에 없는 카드를 입력한 경우
		boolean flag = false;
		boolean result = reasoningInputChecker(userAnswer,"Criminal");
		if(result) {
			flag = criminalCardCheck(userAnswer);
		}else {
	...
	boolean criminalCardCheck(String[] userAnswer) {//범인 카드 정답판별 메서드 호출
		Card card = new Card();
		boolean criminalflag = card.cardCheckerCriminal(userAnswer);
		return criminalflag;
	}
	
	boolean motivationVerification(String[] userAnswer) {//범행동기 카드 목록에 없는 카드를 입력한 경우
		boolean flag = false;
		boolean result = reasoningInputChecker(userAnswer,"Motivation");
		if(result) {
			flag = motivationCardCheck(userAnswer);
		}else {
	...
	boolean motivationCardCheck(String[] userAnswer) { //범행동기 카드 정답판별 메서드 호출
		Card card = new Card();
		boolean motivationflag = card.cardCheckerMotivation(userAnswer);
		return motivationflag;
	}
	
	boolean toolVerification(String[] userAnswer) {//범행도구 카드 목록에 없는 카드를 입력한 경우
		boolean flag = false;
		boolean result = reasoningInputChecker(userAnswer,"Tool");
		if(result) {
	...
	boolean toolCardCheck(String[] userAnswer) {//범행도구 카드 정답판별 메서드 호출
		Card card = new Card();
		boolean toolflag = card.cardCheckerTool(userAnswer);
		return toolflag;
	}
	
	String[] reasoningInputCriminal(int hintcount) {//제거할 범인 카드가 없는 경우('NO')메서드
	...
	String[] reasoningInputMotivation(int hintcount) {//제거할 범행동기 카드가 없는 경우('NO')메서드
	...
	String[] reasoningInputTool(int hintcount) {//제거할 범행도구 카드가 없는 경우('NO')메서드
	...
	void reasoningInput(int hintcount) {//제거할 카드 모두 입력 후 행위 제어 메서드
	...
	void reasoningProcess(String[] userAnswerCriminal, String[] userAnswerMotivation, String[] userAnswerTool, boolean totalflag, int hintcount){//제거 카드 선택 후 삭제 요청메서드
	...
	String[] inputCriminal() {//제거할 범인 카드 입력
		Scanner sc = new Scanner(System.in);
		boolean flag = false;
	...
	String[] inputMotivation() {//제거할 범행동기 카드 입력
		Scanner sc = new Scanner(System.in);
		boolean flag = false;
		String[] userAnswerMotivation = null;
		while(!flag) {
	...
	String[] inputTool() {//제거할 범행도구 카드 입력
		Scanner sc = new Scanner(System.in);
		boolean flag = false;
		String[] userAnswerTool = null;
		while(!flag) {
	...
	boolean inputNullCheck(String[] checkUserInput) {//카드를 입력하지 않은 경우 체크
		boolean flag = true;
	...
	boolean reasoningInputChecker(String[] userInput,String type){//사용자가 입력한 카드의 결과가 추리 성공했는지 판별하는 메서드
	...
	void userDistory() {//게임 종료 메서드
		System.out.println ("게임을 종료합니다.");
		System.exit (0);
	}
	
	void userAnswer() {//정답 보기 메서드
