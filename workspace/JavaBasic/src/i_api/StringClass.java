...
public class StringClass {
	public static void main (String[] args) {
		 ...
		 * - valueOf() : 다른 타입의 데이터를 문자열ㄹ 변환해 반환한다.
		 * */
		
		System.out.println("문자열".equals("문자열"));
		
		String str1 = "문자열";
		String str2 = null;
		System.out.println (str1.equals (str2));
		//주의 ! : null이 있을 수 있는 변수는 앞에두지 않는다.
//		System.out.println (str2.equals (str1));//NullPointerException
		...
		//substring : 문자열 중 원하는 위치부터 자르기
		str = "0123456789";
		String sub1 = str.substring (3);//3번 인덱스부터 끝까지 잘라서 반환한다.
		System.out.println (sub1);
		String sub2 = str.substring (5,8);//5번 인덱스부터 8까지 잘라서 반환한다.
		System.out.println (sub2);
		
		//indesOf : 문자열 중 특정 문자를 찾아 위치를 알려줌
		str = "수박 오렌지 귤 블루베리";
		int index = str.indexOf ("오렌지");//0부터 시작(공백포함)
		...
		for(int i = 0 ; i < menu.length ; i++) {
			System.out.println (menu[i].substring (0,menu[i].indexOf (" ")));
		}
		
		//가격만 잘라서 int 타입의 변수에 저장해주세요
		int[] result = new int[menu.length];
		for(int i = 0 ; i < menu.length ; i++) {
			result[i] = Integer.parseInt (menu[i].substring (menu[i].indexOf (" ")+1,menu[i].indexOf ("원")));
		}
